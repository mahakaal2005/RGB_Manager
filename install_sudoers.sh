...
#   - tee to sysfs fan (fan curve / thermal profile / max fan writes)
...
FAN_TEE_RULE="${REAL_USER} ALL=(ALL) NOPASSWD: /usr/bin/tee /sys/devices/platform/omen-rgb-keyboard/fan/*"
...
printf '%s\n%s\n%s\n%s\n' "$TEE_RULE" "$FAN_TEE_RULE" "$MODPROBE_RM_RULE" "$MODPROBE_ADD_RULE" | sudo tee "$DEST" > /dev/null
